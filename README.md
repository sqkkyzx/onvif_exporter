...
docker run -d \
  ...
  -e CV_WORKER_COUNT=1 \
  -e CV_WORKER_TASK_LIMIT=1 \
  -e ONVIF_MAX_CONCURRENCY=4 \
  -e STREAM_CACHE_TTL=180 \
  -e ANALYSIS_CACHE_TTL=180 \
...
services:
  onvif-exporter:
    ...
    environment:
      # --- 核心性能调优 ---
      - CV_WORKER_COUNT=1           # CV 调度 worker 数
      - CV_WORKER_TASK_LIMIT=1      # 每个 worker 可同时分配的 CV 任务数
      - ONVIF_MAX_CONCURRENCY=4     # ONVIF 网络请求线程并发
      - STREAM_CACHE_TTL=180        # 视频流可读状态缓存寿命 (秒)
      - ANALYSIS_CACHE_TTL=180      # 黑屏/亮度/清晰度/音量等分析结果缓存寿命 (秒)
      - REFRESH_THRESHOLD=120       # 触发后台静默刷新的临界时间 (秒)
      - CV_PROCESS_MAX_TASKS=25     # 每个 CV 子进程处理多少个任务后重建
      - CV_QUEUE_MAXSIZE=2          # CV 后台队列上限，默认 worker slots * 2
      - CV_CACHE_MAX_ENTRIES=256    # CV 缓存最多保留多少个 target
      - CV_CACHE_CLEAN_INTERVAL=120 # CV 过期缓存清理间隔 (秒)
      - BLACK_THRESHOLD=15          # 黑屏判定阈值 (0-255)
      - STRICT_ZERO_PTZ_CHECK=true  # 开启严格 PTZ 零值拦截
      ...
      - ENABLE_BRIGHTNESS=true      # 亮度分析开关
      - ENABLE_CONTRAST=true        # 对比度分析开关
      - ENABLE_COLOR_TEMPERATURE=true # 色温/红蓝比分析开关
...
| 环境变量 | 默认值 | 用法 |
| --- | --- | --- |
| `CV_WORKER_COUNT` | `1` | CV 调度 worker 数。每个 worker 从后台队列领取不同摄像机的刷新任务。 |
| `CV_WORKER_TASK_LIMIT` | `1` | 每个 CV worker 可同时分配的任务槽数。总 CV 并发 = `CV_WORKER_COUNT * CV_WORKER_TASK_LIMIT`，对应进程池最大子进程数。 |
| `ONVIF_MAX_CONCURRENCY` | `4` | ONVIF 协议交互线程并发数。ONVIF 请求主要是网络 I/O，和重内存 CV 分析分开控制。 |
| `STREAM_CACHE_TTL` | `180` | 视频流可读状态缓存寿命，单位秒。该缓存只表示最近是否成功从 RTSP 读到首帧，不等待完整分析结果。 |
| `ANALYSIS_CACHE_TTL` | `180` | 视频分析结果缓存寿命，单位秒。黑屏、亮度、对比度、饱和度、清晰度、音量等慢结果使用该缓存。 |
| `REFRESH_THRESHOLD` | `120` | 视频流状态缓存达到多少秒后触发后台静默刷新，单位秒。建议小于 `STREAM_CACHE_TTL`。 |
...
| `CV_QUEUE_MAXSIZE` | `worker slots * 2` | CV 后台刷新队列最大长度。队列满时会跳过本次后台刷新，避免请求堆积导致内存上涨。 |
| `CV_CACHE_MAX_ENTRIES` | `256` | CV 缓存最多保留多少个 target。适合限制动态 target 或异常请求造成的缓存字典增长。 |
| `CV_CACHE_CLEAN_INTERVAL` | `120` | 清理过期 CV 缓存的间隔，单位秒。流状态缓存按 `STREAM_CACHE_TTL` 清理，分析结果缓存按 `ANALYSIS_CACHE_TTL` 清理。 |
...
| `ENABLE_BRIGHTNESS` | `True` | 是否计算亮度指标；关闭后 `onvif_video_cv_brightness` 返回默认值 `0`，并停用黑屏判定。 |
| `ENABLE_CONTRAST` | `True` | 是否计算对比度指标；关闭后返回默认值 `0`。 |
...
* 2C2G、Wyse 3040 这类边缘节点建议 `CV_WORKER_COUNT=1`、`CV_WORKER_TASK_LIMIT=1`、`CV_QUEUE_MAXSIZE=2`。
* 20 路左右摄像机、CPU/内存余量足够时，可以先试 `CV_WORKER_COUNT=4`、`CV_WORKER_TASK_LIMIT=2`、`CV_QUEUE_MAXSIZE=32`，让不同摄像机并行刷新。
* 默认缓存策略是 stale-while-revalidate：`REFRESH_THRESHOLD=120` 后允许后台刷新，但视频流状态最多复用到 `STREAM_CACHE_TTL=180`，分析结果最多复用到 `ANALYSIS_CACHE_TTL=180`。
...
| `onvif_video_stream_exists` | Gauge | `/probe` | 无 | 是否成功从 RTSP 流中读取首帧。该指标只依赖视频首帧读取结果，不等待音频、亮度、清晰度等完整分析结果。 |
| `onvif_video_stream_cache_valid` | Gauge | `/probe` | 无 | 视频流可读状态缓存是否仍在 `STREAM_CACHE_TTL` 内。`0` 表示当前 `stream_exists` 是默认值或已过期。 |
| `onvif_video_stream_cache_age_seconds` | Gauge | `/probe` | 无 | 视频流可读状态缓存年龄，单位秒；无缓存时为 `-1`。 |
...
| `onvif_exporter_cv_worker_count` | Gauge | `/metrics` | 无 | 当前 CV 调度 worker 数。 |
| `onvif_exporter_cv_worker_task_limit` | Gauge | `/metrics` | 无 | 每个 CV worker 的任务槽数。 |
| `onvif_exporter_cv_worker_slots` | Gauge | `/metrics` | 无 | 当前 CV 总任务槽数，即 `CV_WORKER_COUNT * CV_WORKER_TASK_LIMIT`。 |
...
| `onvif_exporter_stream_cache_ttl_seconds` | Gauge | `/metrics` | 无 | 当前视频流状态缓存 TTL。 |
| `onvif_exporter_analysis_cache_ttl_seconds` | Gauge | `/metrics` | 无 | 当前分析结果缓存 TTL。 |
| `onvif_exporter_cv_cache_entries` | Gauge | `/metrics` | 无 | 当前分析结果缓存 target 数量。保留该旧指标名用于观察总体缓存规模。 |
| `onvif_exporter_cv_stream_cache_entries` | Gauge | `/metrics` | 无 | 当前视频流状态缓存 target 数量。 |
| `onvif_exporter_cv_analysis_cache_entries` | Gauge | `/metrics` | 无 | 当前分析结果缓存 target 数量。 |
  ...
  - name: onvif-exporter.rules
    rules:
      ...
      - alert: OnvifVideoStreamFailed
        expr: |
          (probe_success == 1)
          and on(instance) (avg_over_time(onvif_video_stream_exists[5m]) < 0.5)
          and on(instance) (max_over_time(onvif_video_stream_cache_valid[5m]) == 1)
        for: 2m
        labels:
          severity: warning
        annotations:
          summary: "摄像机 RTSP 视频流读取失败"
          description: "{{ $labels.instance }} ONVIF 正常，但最近 5 分钟 RTSP 首帧读取成功率低于 50%。"

      - alert: OnvifVideoStreamCacheStale
        expr: probe_success == 1 and onvif_video_stream_cache_valid == 0
        for: 10m
        labels:
          severity: info
        annotations:
          summary: "摄像机视频流状态缓存长期未刷新"
          description: "{{ $labels.instance }} ONVIF 正常，但视频流状态缓存超过 STREAM_CACHE_TTL 未刷新，可能是 CV 队列积压或 RTSP 检测过慢。"
